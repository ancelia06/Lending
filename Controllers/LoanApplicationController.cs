...
namespace Lending_CapstoneProject.Controllers
{
    ...
    public class LoanApplicationController : ControllerBase
    {
        ...
        // PUT: api/LoanApplication/{loanApplicationId}/status
        [HttpPut("{loanApplicationId}/status")]
        public async Task<IActionResult> UpdateApplicationStatus(int loanApplicationId, [FromBody] LoanApplicationStatusUpdateDto statusUpdateDto)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            var isUpdated = await _loanApplicationService.UpdateLoanApplicationStatusAsync(loanApplicationId, statusUpdateDto);
            if (!isUpdated)
            {
                return NotFound("Loan application not found or unauthorized.");
            }
            return Ok("Loan application status updated successfully.");
        }
